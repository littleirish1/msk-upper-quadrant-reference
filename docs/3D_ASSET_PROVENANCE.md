...
| `Z-Biomechanics.7z` | 21,644,942 bytes; SHA-256 `0a74b2fa3c47db925b06241fa38c89b0811e58a78ee28ec440cd460e279cf22b`; independently reproduced Git blob `0a59584deaf21773ffa3acb063f2cd06f3485a98` at the same pinned revision; Microsoft Defender found no threats | The exact upstream archive contains `Startup.blend` with a Blender 3.00 header. Its introduction commit describes a WIP first attempt at a biomechanics rig. Twenty-two of 26 local movement node strings occur literally in that Blend file. | No local GLB is byte-identical to an upstream file. `Startup.glb`, `Startupbio.glb`, build/export scripts, exact commands, settings and tool lock are absent. Literal node-name correspondence and the WIP description are context only, not derivation proof or clinical evidence. | false |
...
The private candidate pipeline deliberately separates three archive/source-package records from five exact derived-GLB records and 18 movement-definition records. `upstream-comparison.json` records complete deterministic inventories and the absence of any exact or renamed binary match between the biomechanics archives. Each derived GLB retains its own hash, parent archive, declared lineage, GLB scene/node/named-node/root-node/mesh/primitive/material/skin/animation counts, encoded bounds, and empty external-resource list. All nodes have names and the five files contain no animation clips, but technical GLB validity is not anatomical or movement approval. Every GLB provenance outcome remains `UNRESOLVED`.

`attribution-template.json` records the private attribution data needed for a future asset-specific package. The licence family is verified as CC BY-SA 4.0 with the reviewed Z-Anatomy and BodyParts3D attribution requirements and ShareAlike/change-disclosure obligations. Exact-asset licence lineage remains unresolved, modification descriptions are blank, and differently licensed component exclusion remains required. No licensing approval is granted.

`scripts/private-review-portal/inspect-glb.mjs` provides dependency-free technical inspection of a privately held GLB path. It validates the GLB 2.0 container and reports scene, node, hierarchy, mesh, primitive, material, skin, animation, resource and bounds metadata. It does not write the file, execute content or create a public preview. No vetted private renderer is present, so rotate/pan/zoom and anatomical visual review remain explicit human tasks rather than an implied capability.
