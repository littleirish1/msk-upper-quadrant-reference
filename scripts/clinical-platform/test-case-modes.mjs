...
assert.ok(component.includes('onKeyDown={submitQuestionOnEnter}'), 'conversation composer must support Enter-to-send')
assert.ok(component.includes("event.shiftKey || event.nativeEvent.isComposing"), 'composer must preserve Shift+Enter and IME composition')
assert.ok(component.includes('event.currentTarget.form?.requestSubmit()'), 'Enter-to-send must use the existing form submission path')
assert.equal((component.match(/<CaseReasoningPrompt/g) ?? []).length, 1, 'Guided reasoning must render only in the Guided panel')
assert.ok(!component.includes("mode !== 'guided'"), 'Conversation and Hybrid must not retain a hidden Guided panel')
assert.ok(component.includes('{guidedPresentation}'), 'case presentation must be owned by the Guided panel')
assert.ok(page.includes('guidedPresentation={casePresentationContent ? ('), 'case presentation must be passed into the mode switcher')
